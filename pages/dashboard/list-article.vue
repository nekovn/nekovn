<template>
  <div>
    ...
    <a-drawer  v-if="display ==='edit'" placement="right" :closable="false" :visible="visible" @close="onClose">
      <drawer-article :content="content" :id="id" :title="title" :categories="categories" :image="image" @close="onClose"/>
    </a-drawer>
  </div>
</template>
<script>
...
export default {
  ...
  async asyncData({params, store}) {
    ...
    await store.dispatch('posts/actFetchArticlesByUser', {
      author: id,
    });
  ...
  mounted() {
    const {post, category} = this;
    if (post) {
      this.dataSource = post.map((obj, index) => {
        if (obj.title && obj.content && obj.categories) {
          let arrCategories = [];
          category.map((val, key) => {
            if (obj.categories[0] === val.id) {
              arrCategories += val.name;
            }
          })
  ...
  computed: {
    ...mapState({
      post: state => state.posts.relatedPosts,
      category: state => state.category.categoriesList
